...
Usage:
    python extract_alameda_data_v2.py
"""
...
def main():
    ...
    # ========================================================================
    # TABLE 1: filername_cd - Master Filer Registry
    # ========================================================================
    query = f"""
    SELECT
        "FILER_ID",
    ...
    """
    count = export_table(query, 'Alameda_Filers.csv',
    ...
    """
    count = export_table(query, 'Alameda_Lobby_Disclosures.csv',
    ...
    """
    count = export_table(query, 'Alameda_Registrations.csv',
    ...
    LEFT JOIN `{PROJECT_ID}.{DATASET_ID}.filername_cd` f ON lp."FILER_ID" = f."FILER_ID"
    ...
    """
    count = export_table(query, 'Alameda_Payments.csv',
    ...
    LEFT JOIN `{PROJECT_ID}.{DATASET_ID}.filername_cd` f ON le."FILER_ID" = f."FILER_ID"
    ...
    """
    count = export_table(query, 'Alameda_Expenditures.csv',
    ...
    """
    count = export_table(query, 'Alameda_Employers.csv',
    ...
    LEFT JOIN `{PROJECT_ID}.{DATASET_ID}.filername_cd` f ON lc."FILER_ID" = f."FILER_ID"
    ...
    """
    count = export_table(query, 'Alameda_Campaign_Contributions.csv',
    ...
    LEFT JOIN `{PROJECT_ID}.{DATASET_ID}.filername_cd` f ON lo."FILER_ID" = f."FILER_ID"
    WHERE lo."FILER_ID" IN (SELECT "FILER_ID" FROM alameda_filers)
       OR UPPER(lo."PAYEE_NAML") LIKE '%ALAMEDA%'
    ORDER BY lo."FILING_ID"
    """
    count = export_table(query, 'Alameda_Other_Payments.csv',
                         'Other Lobbying Payments')
    total_records += count
    tables_extracted += 1

    # ========================================================================
    # TABLE 9: latt_cd - Lobbyist Activity
    # ========================================================================
    query = f"""
    WITH alameda_filers AS (
        SELECT DISTINCT "FILER_ID"
    ...
    SELECT
        ...
        la."FILER_ID",
        f."FILER_NAML" as FILER_ORGANIZATION,
        la."LBY_NAML" as LOBBYIST_LAST_NAME,
        la."LBYF_NAMF" as LOBBYIST_FIRST_NAME,
        la."FORM_TYPE",
        'latt_cd' as SOURCE_TABLE
    FROM `{PROJECT_ID}.{DATASET_ID}.latt_cd` la
    LEFT JOIN `{PROJECT_ID}.{DATASET_ID}.filername_cd` f ON la."FILER_ID" = f."FILER_ID"
    WHERE la."FILER_ID" IN (SELECT "FILER_ID" FROM alameda_filers)
       OR UPPER(la."LBY_NAML") LIKE '%ALAMEDA%'
    ORDER BY la."FILING_ID"
    """
    count = export_table(query, 'Alameda_Lobbyist_Activity.csv',
                         'Lobbyist Activity Records')
    total_records += count
    tables_extracted += 1
    ...
    payment_summary AS (
        SELECT
            ...
            SUM(CAST(lp."FEES_AMT" AS FLOAT64)) as TOTAL_FEES,
            SUM(CAST(lp."REIMB_AMT" AS FLOAT64)) as TOTAL_REIMBURSEMENTS
        FROM `{PROJECT_ID}.{DATASET_ID}.lpay_cd` lp
        WHERE lp."FILER_ID" IN (SELECT "FILER_ID" FROM alameda_filers)
    ...
    expenditure_summary AS (
        SELECT
            ...
            SUM(CAST(le."AMOUNT" AS FLOAT64)) as TOTAL_EXPENDITURES
        FROM `{PROJECT_ID}.{DATASET_ID}.lexp_cd` le
        WHERE le."FILER_ID" IN (SELECT "FILER_ID" FROM alameda_filers)
