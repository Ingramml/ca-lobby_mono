...
- **Direct BigQuery backend** - Web application queries BigQuery views directly (no CSV exports)
---
...
- ✅ Direct BigQuery backend connection
...
```
BigQuery → CSV Export → Sample data/*.csv → Python Scripts →
JSON files (src/data/*.json) → React Components → UI
```
...
```
BigQuery Tables → BigQuery Views → Flask API Endpoints →
React Frontend → User Interface
```

**Data Flow**:
1. **User searches** for organization in React frontend
2. **Frontend calls** Flask API: `GET /api/organizations?search=hospital`
3. **Flask backend queries** BigQuery view: `SELECT * FROM v_org_profiles_complete WHERE organization_name LIKE '%hospital%'`
...
-- Frontend application can filter by city, county, organization name, etc.

ORDER BY d.period_start_date DESC;
```

**Key Features**:
- **No geographic filtering** - Returns all California lobbying data
- **Joins by EMPLOYER** (the organization), not by FILER (the lobbying firm)
- **Includes location fields** (city, state, zip) for frontend filtering
- **Web application handles filtering** based on user search criteria
...
-- NO WHERE CLAUSE - Returns ALL lobbyist relationships statewide
-- Frontend application filters by organization
...
-- NO geographic filtering - Frontend handles filtering by organization
...
-- NO WHERE CLAUSE - Returns ALL expenditure data statewide
-- Frontend application filters as needed
...
1. **Import Core Tables to BigQuery** (if not already imported):
   ```bash
   ...
   bq load --source_format=CSV \
     ca_lobby.CVR_REGISTRATION_CD \
     CVR_REGISTRATION_CD.txt

   bq load --source_format=CSV \
     ca_lobby.TEXT_MEMO_CD \
     TEXT_MEMO_CD.txt
   ```

2. **Create Optimized Views in BigQuery**:
   - Execute SQL for View 1: `v_org_profiles_complete`
   - Execute SQL for View 2: `v_lobbyist_network`
   - Execute SQL for View 3: `v_activity_timeline`
   - Execute SQL for View 4: `v_expenditure_categories`

3. **Configure Web Application Backend**:
   - Update Flask backend to connect to BigQuery
   - Configure BigQuery client credentials
   - Create API endpoints that query the views directly
   - **No CSV export needed** - Direct database queries
...
3. Connect web app directly to BigQuery views via Flask backend
4. Frontend queries backend API → Backend queries BigQuery → Returns JSON to frontend
...
### Step 4: Configure Flask Backend for BigQuery
- [ ] Install BigQuery Python client library
- [ ] Configure service account credentials
- [ ] Create API endpoints for each view
- [ ] Add query parameters for filtering (organization, date range, etc.)
- [ ] Implement pagination for large result sets
- [ ] Add caching layer for performance

### Step 5: Update Frontend Application
- [ ] Remove CSV-based data loading
- [ ] Update components to call backend API
- [ ] Test lobbyist network component (now has data)
- [ ] Verify timeline sorting works (now has dates)
